...
> **중요: 사용자 이름은 반드시 `vscode`로 입력하세요.**
> 6단계에서 사용할 컨테이너 베이스 이미지(`mcr.microsoft.com/devcontainers/base:ubuntu`)에는 `vscode`라는 사용자가 이미 만들어져 있습니다. WSL 사용자 이름이 이와 다르면 바인드 마운트된 `.claude_storage` 폴더의 권한이 어긋나 컨테이너가 파일을 읽고 쓰지 못하게 됩니다.

> **비밀번호는 입력해도 화면에 표시되지 않습니다 (정상)**. 잊지 않게 메모해두세요. 이 비밀번호는 나중에 `sudo` 명령어 사용할 때 입력해야 합니다.

설정 끝나면 Ubuntu 터미널이 열린 상태로 대기합니다 (`vscode@<PC이름>:~$` 프롬프트가 보임).
